...
typedef struct RF24_data_t
{
	uint32_t	ce_periph;
	uint32_t	ce_pin_base;	/**< "Chip Enable" pin, activates the RX or TX role */
	uint8_t		ce_pin;
	uint32_t	csn_periph;
	uint32_t	csn_pin_base;	/**< SPI Chip select */
	uint8_t		csn_pin;
...
//$$ protected:
  ...
  void delayMicroseconds( uint32_t msec );
   ...
   */
  void RF24_print_byte_register(const char* name, uint8_t reg, uint8_t qty /* = 1 */ );
   ...
   * @param qty How many successive registers to print
   */
  void RF24_print_address_register(const char* name, uint8_t reg, uint8_t qty /* = 1 */ );
...
//$$ public:
   ...
   */
  bool RF24_isValid(void); //$$ { return ce_pin != 0xff && csn_pin != 0xff; } 
